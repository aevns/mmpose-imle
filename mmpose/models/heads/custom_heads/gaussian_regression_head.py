...
class GaussianRegressionHead(BaseHead):
    ...
    def __init__(self,
                 ...
                 loss: ConfigType = dict(
                     type='NLLGaussianLoss', use_target_weight=True),
    ...
    def forward(self, feats: Tuple[Tensor]) -> Union[Tensor, Tuple[Tensor]]:
        ...
        # Alternative to _flat_softmax;should be more numerically stable.
        # Ignoreing self.beta.
        #heatmaps = self._flat_softmax(feats * self.beta)
        with torch.no_grad():
            max_pd = torch.max(torch.max(feats, dim=-1)[0], dim=-1, keepdim=True)[0].unsqueeze(-1)
        z = torch.sum(torch.exp(feats - max_pd), (2, 3)).view(B, N, 1, 1)
        heatmaps = torch.exp(feats - max_pd) / z

        x_vals = torch.linspace(0, W-1, W, device=heatmaps.device).unsqueeze(0)
        y_vals = torch.linspace(0, H-1, H, device=heatmaps.device).unsqueeze(1)

        # Calculating all relevant stats for the gaussian fit instead of just means.
        #pred_x = self._linear_expectation(heatmaps, self.linspace_x)
        #pred_y = self._linear_expectation(heatmaps, self.linspace_y)
        x_means = torch.sum(heatmaps * x_vals, dim = (2, 3))
        y_means = torch.sum(heatmaps * y_vals, dim = (2, 3))
        ...
        presence_prob = 1 - 1 / (torch.exp(max_pd) * z / (H*W) + 1).view(B, N)

        keystats = torch.stack((x_means/W, y_means/H, x_var/(W*W), y_var/(H*H), xy_covar/(W*H), presence_prob), -1)
        return keystats, heatmaps
