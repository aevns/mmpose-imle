...
class HRNetND(BaseBackbone):
    ...
    def init_weights(self):
        ...
        super(HRNetND, self).init_weights()
    ...
    def forward(self, xz):
        ...
        tmp = cat((y_list[2], z.view(-1,self.noise_channels,1,1).expand(y_list[2].shape[0], -1, y_list[2].shape[2], y_list[2].shape[3])), dim=1)
        tmp = self.conv3(tmp)
        tmp = self.norm3(tmp)
        tmp = self.relu(tmp)
        y_list[2] = tmp
